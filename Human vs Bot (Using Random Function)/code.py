...
def possible_moves_red(pawns):
    moves={}
    for idx in pawns:
        if idx in board_index:
            a=board_index[idx][0]
            b=board_index[idx][1]
            if (a-100>=0 and b+100<800):
                blk=find_block((a-100,b+100))
                if blk!=-1 and (blk not in pawns):
                    if idx in moves:
                        moves[idx].append(blk)
                    else:
                        moves[idx]=[blk]
            if (a+100<800 and b+100<800):
                blk=find_block((a+100,b+100))
                if blk!=-1 and (blk not in pawns):
                    if idx in moves:
                        moves[idx].append(blk)
...
def choosing_move(moves):
    res = key, val = random.choice(list(moves.items())) 
    print(res)
    random_num = random.choice(val)
    print(random_num) 
    return (key,random_num)
...
def main():
    ...
    while True:
        if(not bool(red_pawn) and not bool(blue_pawn)):
            ...
            moves=possible_moves_blue(blue_pawn)
            fro,to=choosing_move(moves)
            while(moves!={} and move_pawn(fro,to,(0,255,255),Window)==-1):
                fro,to=choosing_move(moves)
                moves[fro].remove(to)
                if(moves[fro]==[]):
                    del moves[fro]
            ...
            moves=possible_moves_red(red_pawn)
            fro,to=choosing_move(moves)
            while(moves!={} and move_pawn(fro,to,(255,0,0),Window)==-1):
                fro,to=choosing_move(moves)
                moves[fro].remove(to)
                if(moves[fro]==[]):
                    del moves[fro]
